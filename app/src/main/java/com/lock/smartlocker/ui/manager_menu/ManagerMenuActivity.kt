    ...
    private fun initView() {
        when (intent.getStringExtra(ConstantUtils.TYPE_OPEN_MANAGER)) {
            ConstantUtils.TYPE_REGISTER_FACE -> {
                showFragment(R.navigation.register_face_navigation)
            }

            ConstantUtils.TYPE_MANAGER_FACE -> {
                showFragment(R.navigation.admin_console_navigation, typeManager = ConstantUtils.TYPE_MANAGER_FACE)
            }

            ConstantUtils.TYPE_ADMIN_CONSOLE -> {
                showFragment(R.navigation.admin_console_navigation, typeManager = ConstantUtils.TYPE_ADMIN_CONSOLE)
            }
        }
    }
}